...
def extract_song_info(title: str, uploader: str = "") -> tuple:
    ...
    clean_title = re.sub(
        r'\s*[-–]\s*(HD|4K|Official|Music Video|Live|Remastered|Lyrics|Audio|Cover|Tribute|Instrumental|Karaoke|Topic|VEVO)\s*$',
        '',
        clean_title,
        flags=re.IGNORECASE
    )
    ...
    if artist_name == uploader:
        artist_name = re.sub(
            r'\s*(Topic|VEVO|Official|Channel|Music|Records|Label|Entertainment|Studio)\s*$',
            '',
            artist_name,
            flags=re.IGNORECASE
        )
    ...
    song_name = re.sub(
        r'\s*(Official Music Video|Official Video|Music Video|HD|4K|Live|Remastered|Lyrics|Audio|Cover|Tribute|Instrumental|Karaoke|Topic|VEVO)\s*$',
        '',
        song_name,
        flags=re.IGNORECASE
    )
...
def process_song(youtube_url: str, song_folder: str):
    ...
    ydl_opts = {
        ...
        # ========================================================
        #
        # O bgutil instalado no Docker fica em:
        #
        # /opt/bgutil-ytdlp-pot-provider/server
        #
        # O provider gera automaticamente os PO Tokens
        # necessários para as requisições do YouTube.
        #
        "extractor_args": {
            "youtube": {
                "player_client": ["mweb"]
    ...
    try:
        ...
        print("[Guitar Livre] Obtendo dados e baixando vídeo...")
        ...
        if not mp4_files:
            ...
            if os.path.exists(video_path):
                ...
                raise FileNotFoundError(
                    "Nenhum arquivo MP4 foi encontrado após o download."
                )
            ...
            print(
                f"[Guitar Livre] "
                f"Vídeo encontrado: {downloaded_video}"
            )
        ...
        if video_size_mb > MAX_VIDEO_SIZE_MB:
            ...
            raise ValueError(
                f"O vídeo baixado excede o limite de "
                f"{MAX_VIDEO_SIZE_MB} MB "
                f"({video_size_mb:.1f} MB). "
                f"Por favor, escolha um vídeo menor."
            )
        ...
        if not os.path.exists(audio_path):
            ...
            raise FileNotFoundError(
                "O FFmpeg terminou, mas o audio.wav "
                "não foi criado."
            )

        print(
            f"[Guitar Livre] "
            f"Áudio extraído: {audio_path}"
        )
        ...
        print(
            "[Guitar Livre] "
            "Etapa 5/7 - Buscando chart no Enchor..."
        )
        ...
        song_title, song_artist = extract_song_info(
            raw_title,
            uploader
        )
        ...
        print(
            f"[Guitar Livre] "
            f"Música extraída: {song_title}"
        )

        print(
            f"[Guitar Livre] "
            f"Artista extraído: {song_artist}"
        )
        ...
        if not chart_converted:
            ...
            print(
                "[Guitar Livre] "
                "Nenhuma chart do Enchor encontrada."
            )

            print(
                "[Guitar Livre] "
                "Compondo chart determinística pelo áudio..."
            )
        ...
        if os.path.exists(metadata_path):
            ...
            try:
                ...
                print(
                    "[Guitar Livre] "
                    f"Erro ao atualizar metadata: {e}"
                )
